...
from scraper.prozorro.spiders.tenders import TendersSpider


class EtendersSpider(TendersSpider):
    name = 'etenders'
    ...
    def parse_tender(self, response: TextResponse):
        payload = response.json()
        # reduce tenders to only eauctions
        if payload['data'].get('submissionMethod', '') == 'electronicAuction':
            yield payload